...
program
	...
	.option("--base-url <url>", "Base URL for OpenAI-compatible API endpoints (e.g. http://localhost:8000/v1)")
	.option("--context-window <size>", "Context window size override", (value) => Number.parseInt(value, 10))
	.option("--temperature <temp>", "Model temperature (e.g. 0.7)", (value) => Number.parseFloat(value))
	.option("--top-p <value>", "Top-p (nucleus) sampling (e.g. 0.8)", (value) => Number.parseFloat(value))
	.option("--top-k <value>", "Top-k sampling (e.g. 20)", (value) => Number.parseInt(value, 10))
