...
#
# Roo Test: all languages — Part 3b (Qwen3-Next 80B-A3B MoE, baseline merges)
#
# Models (all derived from Qwen3-Next-80B-A3B Instruct × Thinking, α=0.15):
#   1. qwen3-next-80b-ta        — Task Arithmetic
#   2. qwen3-next-80b-ties      — TIES (density=0.5)
#   3. qwen3-next-80b-slerp     — SLERP (t=0.15)
#   4. qwen3-next-80b-aim-ta    — AIM + Task Arithmetic (ω=0.4)
#   5. qwen3-next-80b-aim-ties  — AIM + TIES (ω=0.4)
#   6. qwen3-next-80b-lewis     — LEWIS merge
#
...
#   - TP=4 alone would shard 512 tiny experts (moe_intermediate_size=512)
#     into micro-GEMMs; expert-parallel keeps each expert whole and
#     shards the 512 experts across ranks (128/GPU) — far more efficient.
#   - Weights ≈40 GB/GPU in bf16; ~40 GB/GPU left for KV cache at gmu=0.90.
#
# Languages: python, javascript, go, java, rust (195 exercises total)
# Iterations: 3, temperature: 0.6
#
# Usage:
#   sbatch sh/run_all_languages_3_b.sh                   # full run
#   bash sh/run_all_languages_3_b.sh --limit 3           # smoke-test
#   bash sh/run_all_languages_3_b.sh --language python   # single language
...
NUM_MODELS=${#MODELS[@]}
sep "All-Languages Eval Part 3b: ${NUM_MODELS} models × ${#LANGUAGES[@]} languages (port ${VLLM_PORT})"
...
for entry in "${MODELS[@]}"; do
    ...
    local_log="vllm-allang3b-${model_name}.log"
    ...
    for lang in "${LANGUAGES[@]}"; do
        ...
        tmp_config="/tmp/eval-allang3b-${model_name}-${lang}.yaml"
        cat > "$tmp_config" <<YAML
provider: openai
model: "${model_name}"
api_key: "EMPTY"
