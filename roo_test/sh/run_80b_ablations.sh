...
#SBATCH -J roo-80b-ablation
#SBATCH -o ${CRANE_LOG_DIR}/sbatch_log/%x-%j.out
#SBATCH -e ${CRANE_LOG_DIR}/sbatch_log/%x-%j.err

#
# 80B canonical TGSP ablation eval — same config as canonical except for
# one --no-* flag flipped per variant.
#
# Models (all share α=0.15, τ=0.03):
#   1. crane-next-80b-noT      — drops T(δ) median denoise
#   2. crane-next-80b-noGSP    — drops Π_τ format-nullspace
#   3. crane-next-80b-noTaylor — flat α=0.15 (no per-layer S_reason)
#
# Languages: python, javascript, go, java, rust (195 exercises × 3 iter)
# Sampling: temp 0.6, top_p 0.8, top_k 20, ctx 90k, conc 64, timeout 300/900
#
# Usage:
#   sbatch sh/run_80b_ablations.sh
#   bash   sh/run_80b_ablations.sh                  # foreground
#   bash   sh/run_80b_ablations.sh --language java   # single language
#
...
NUM_MODELS=${#MODELS[@]}
sep "canonical TGSP ablations: ${NUM_MODELS} models × ${#LANGUAGES[@]} languages (port ${VLLM_PORT})"
...
for entry in "${MODELS[@]}"; do
    ...
    local_log="vllm-3bm15ab-${model_name}.log"
    start_vllm "$model_path" "$model_name" "$local_log"

    for lang in "${LANGUAGES[@]}"; do
        log "── Eval: ${model_name} / ${lang} ──"

        # Cleanup leaked harness processes between languages.
        ...
        tmp_config="/tmp/eval-3bm15ab-${model_name}-${lang}.yaml"
        cat > "$tmp_config" <<YAML
provider: openai
model: "${model_name}"
api_key: "EMPTY"
...
if isinstance(d, dict) and d.get('overall',{}).get('model','') == '$model_name':
    g = d.get('by_language', {}).get('$lang')
    if g and 'pass_at_1' in g:
        print(f\"{g['pass_at_1']}/{g['exercises']}\")
        sys.exit(0)
" 2>/dev/null)
            if [[ -n "$result" ]]; then
                printf "  %-12s" "$result"
